...
class movielist {//MOVIE LIST CLASS
   ...
   Boolean CheckAvalability(int passid, int noOfTickets) {// CHEAK THE VALID MOVIE ID NO AND SEAT CAPACITY
      int seatavlable = 0;
   ...
   void MovieDisplay() {// DISPLAY THE AVALABLE MOVIE LIST 
      Connection conn = null;
      Statement stmt = null;
      try {
   ...
   void DisplayCustomerListDetails(Integer[] myMovieList, Integer[] myNoOfTickets) {// DISPLAY THE CUSTOMER MOVIE LIST THAT THEY CHOOSE
	   System.out.println("\n\tConnecting to database...");
	   System.out.println("\n\tFetching data from database....\nYour List \n");
   ...
   void CustomerCheckOut(Integer[] myMovieList, Integer[] myNoOfTickets) { //CHECKOUT THE CUSTOMER GENARATE BILL 
      int finalprice = 0;
      for (int i = 0; i < myMovieList.length; i++) {
         if (myNoOfTickets[i] == -9999) {
            continue;
         ...
         try {
            ...
            while (rs.next()) {
               ...
               int moviehallcapacity = moviecap - myNoOfTickets[i];// UPDATE THE DATABASE AFTER SUCCESSFULL CHECKOUT
               stmt = conn.createStatement();
...
class Customer { //CUSTOMER CLASS
   ...
   void AddToMyList(int myMovieId, int noOfTickets, movielist movies) { // ADD THE MOVIE IN CUSTOMER LIST
   ...
   void RemoveFromList(int myMovieid, int noOfTickets) { // REMOVE PRODUCT FROM CUSTOMER LIST
      int b = 0;
